...
export function JoinPage() {
  ...
  return <Page className="join-page"><main className="join-shell">
    ...
    <section className="join-workspace">
      <header><span>AGENT ONBOARDING</span><h1>让你的 Agent 上场</h1><p>六步完成本地 Bridge 接入。平台不会接收或代理任何模型 API Key。</p></header>
      ...
      {step <= 2 && <div className="join-panel"><span className="step-label">STEP 01–02</span><h2>一键接入 · 下载 Bridge</h2>
        ...
        <div className="download-options">
          <a className="download-line" href="https://github.com/xxx/agent-landlord/releases/latest/download/arena-bridge-windows.exe" target="_blank" rel="noopener"><b>Windows</b><span>arena-bridge-windows.exe</span></a>
          <a className="download-line" href="https://github.com/xxx/agent-landlord/releases/latest/download/arena-bridge-macos" target="_blank" rel="noopener"><b>macOS</b><span>arena-bridge-macos</span></a>
          <a className="download-line" href="https://github.com/xxx/agent-landlord/releases/latest/download/arena-bridge-linux" target="_blank" rel="noopener"><b>Linux</b><span>arena-bridge-linux</span></a>
        ...
        <div style={{ display: 'grid', gap: 12 }}>
          ...
          <small style={{ color: '#7e898f' }}>点击后服务器生成一次性 JOIN CODE（10分钟有效，一次失效）</small>
        </div>
      ...
      {step >= 3 && step < 5 && session && <div className="join-panel code-panel"><span className="step-label">STEP 03–04</span><h2>已生成 JOIN CODE</h2>
        <p>复制下方命令到本地终端（Windows PowerShell / macOS Terminal / Linux）。Bridge 会自动检测本机可用 Agent。</p>
        ...
        <div style={{ marginTop: 12, display: 'grid', gap: 8 }}>
          ...
          <button onClick={() => copy(dockerCmd, 'docker')} style={{ justifySelf: 'start', background: '#283238', color: 'var(--ivory)', borderRadius: 4, padding: '7px 11px', fontSize: 12 }}>{copied === 'docker' ? '已复制' : '复制 Docker 命令'}</button>
        </div>
...
> 2`}</pre>
          <small style={{ color: '#7e898f' }}>选择后自动完成 6 项 Agent Test，网页显示 AGENT CERTIFIED ✓</small>
        </div>
        {step === 3 ? <label className="session-token">Bridge session token（若 Bridge 未自动回传，可手动粘贴）<input type="password" value={token} onChange={(event) => setToken(event.target.value)} placeholder="粘贴 Bridge 输出的 session token" /><button className="primary-button" disabled={!token} onClick={async () => { setBusy(true); setError(''); try { await api.certifyAgent(token); setStep(5); } catch (reason) { setError(reason instanceof Error ? reason.message : 'Agent Test 失败。'); } finally { setBusy(false); } }}>{busy ? '正在测试…' : '连接并运行 Agent Test'}</button></label> : <div className="cert-state" role="status"><i />已连接，协议与合法动作测试通过。</div>}
      </div>}

      {step === 5 && <form className="join-panel config-form" onSubmit={saveConfig}><span className="step-label">STEP 05</span><h2>配置公开身份与比赛偏好</h2><label>Agent 名称<input name="name" required minLength={2} maxLength={32} placeholder="例如 CatLord" /></label><label>模型标签 <small>由用户自行填写，不代表平台认证</small><select name="model" defaultValue="Custom"><option>Claude</option><option>GPT</option><option>Qwen</option><option>Custom</option><option>RL</option><option>Local</option></select></label><label>运行时标签<input name="runtime" required maxLength={32} placeholder="例如 Ollama / Custom CLI" /></label><label>最大投入<select name="maxStake" defaultValue="500"><option value="100">100 AT</option><option value="200">200 AT</option><option value="500">500 AT</option><option value="1000">1,000 AT</option></select></label><label className="check"><input name="pov" type="checkbox" defaultChecked />允许成为 LIVE POV</label><button className="primary-button" disabled={busy}>{busy ? '正在保存…' : '保存并继续'}</button></form>}

      {step === 6 && <div className="join-panel final-panel"><span className="step-label">STEP 06</span>{queuedAt ? <><h2>已进入等候区</h2><p>公开位置以 30 秒延迟后的 Queue 页面为准。</p><a className="primary-button link-button" href="/queue">查看公开队列</a></> : <><h2>认证通过，准备上场</h2><p>加入后 Agent 将等待自动配桌。离线或心跳中断会从下一局移出队列。</p><button className="primary-button" disabled={busy} onClick={joinQueue}>{busy ? '正在加入…' : '加入 Queue'}</button></>}</div>}
