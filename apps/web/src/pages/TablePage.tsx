  ...
  return <Page className="table-page">
    <main className="broadcast-stage" aria-label="直播牌桌">
      <section className="table-header">
        <div><span>LIVE TABLE</span><strong>第 {table.handNo} 局</strong><small>{table.gameId}</small></div>
        <div className="match-state"><span>{table.status}</span><strong>×{table.multiplier}</strong><small>当前倍数</small></div>
        <div className="stake-block"><span>BASE STAKE</span><strong>{formatAt(table.baseStake)}</strong><small>最大投入 {formatAt(Math.min(table.baseStake * 8, 4000))}</small></div>
      </section>
      ...
      <section className="table-field">
        ...
        <div className="center-play">
          ...
          <div className="last-trick" aria-label="最近出牌">
            {table.history.at(-1)?.type === 'PASS' ? <strong className="pass-word">PASS</strong> : table.history.at(-1)?.cards.map((card) => <PlayingCard key={card} card={card} />)}
          </div>
      ...
      <section className="pov-dock">
        <div className="pov-plate"><span className="live-pov">LIVE POV</span><AgentPlate agent={center} active={table.turnAgentId === center.id} /><blockquote>{center.comment ? `“${center.comment}”` : '公开策略说明暂缺'}</blockquote></div>
      ...
      <aside className="history-rail" aria-label="出牌历史">
        <div className="history-title"><span>EVENT HISTORY</span><strong>出牌记录</strong></div>
        <ol>{table.history.slice(-6).reverse().map((action) => <li key={action.id}><span>{String(action.sequence).padStart(3, '0')}</span><strong>{action.actor}</strong><em>{action.type === 'PASS' ? 'PASS' : action.cards.join(' ')}</em></li>)}</ol>
        <div className="round-meta"><span>胜方连胜</span><b>{Math.max(...table.agents.map((a) => a.streak ?? 0))} WIN</b><span>零筹码</span><b>自动淘汰</b></div>
      </aside>
